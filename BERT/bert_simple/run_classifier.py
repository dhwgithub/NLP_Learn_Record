...
def convert_example_list_for_inews(ex_index, example, label_list, max_seq_length,
    ...
    if example.text_b and extra_len > 0:
        ...
        for num in range(extra_num):
            ...
            feature = convert_single_example_for_inews(ex_index, tokens_a, tokens_b_sub, label_map, max_seq_length,
                                                       tokenizer, example)
            feature_list.append(feature)
    else:
        feature = convert_single_example_for_inews(ex_index, tokens_a, tokens_b, label_map, max_seq_length, tokenizer,
                                                   example)
        feature_list.append(feature)
    return feature_list
...
def main(_):
    ...
    # is_per_host = True  # per_host_input_for_training只有true和false两种取值
    ...
    if FLAGS.do_eval:
        ...
        tf.logging.info('eval num = ' + str(num_actual_eval_examples))
        if FLAGS.use_tpu:
        ...
        steps_and_files = []
        filenames = tf.gfile.ListDirectory(FLAGS.output_dir)  # 获取当前目录所有文件列表（不包含子目录）
        ...
        tf.logging.info("filenames num is = " + str(len(filenames)))
        for filename in filenames:
            if filename.endswith(".index"):
                ckpt_name = filename[:-6]
                cur_filename = os.path.join(FLAGS.output_dir, ckpt_name)
                ...
                steps_and_files.append([global_step, cur_filename])
        steps_and_files = sorted(steps_and_files, key=lambda x: x[0])
        tf.logging.info("sorted(steps_and_files, key=lambda x: x[0]) num is = " + str(len(steps_and_files)))

        # output_eval_file = os.path.join(FLAGS.data_dir, "test_results_bert.txt")
        output_eval_file = os.path.join(r'D:\Downloads\tnews', "test_results_bert.txt")
        print("output_eval_file:", output_eval_file)
        tf.logging.info("output_eval_file:" + output_eval_file)
        with tf.gfile.GFile(output_eval_file, "w") as writer:
            tf.logging.info("steps_and_files num is = " + str(len(steps_and_files)))
            for global_step, filename in sorted(steps_and_files, key=lambda x: x[0]):
                result = estimator.evaluate(input_fn=eval_input_fn, steps=eval_steps, checkpoint_path=filename)

                tf.logging.info("***** Eval results %s *****" % (filename))
                writer.write("***** Eval results %s *****\n" % (filename))
                tf.logging.info("writer finish !!!")
                for key in sorted(result.keys()):
                    tf.logging.info("  %s = %s", key, str(result[key]))
                    writer.write("%s = %s\n" % (key, str(result[key])))
                    tf.logging.info("writer finish !!!")
    ...
    if FLAGS.do_predict:
        ...
        tf.logging.info("  predict num is = %d", num_actual_predict_examples)
        if FLAGS.use_tpu:
            # TPU requires a fixed batch size for all batches, therefore the number
            # of examples must be a multiple of the batch size, or else examples
            # will get dropped. So we pad with fake examples which are ignored
        ...
        output_predict_all = os.path.join(FLAGS.output_dir, "test_results_all.tsv")  # 自己写的
        with tf.gfile.GFile(output_predict_all, "w") as writer_all:  # 自己写的
            with tf.gfile.GFile(output_predict_file_label, "w") as writer_label:
                with tf.gfile.GFile(output_predict_file, "w") as writer:
                    ...
                    for (i, prediction) in enumerate(result):
                        ...
                        writer_all.write(dicts[int(index2label_map[label_index])]
                                         + "\t" + str(index2label_map[label_index])
                                         + "\n")  # 自己写的
                        num_written_lines += 1
