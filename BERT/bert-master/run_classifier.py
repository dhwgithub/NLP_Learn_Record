...
def convert_single_example(ex_index, example, label_list, max_seq_length,
  ...
  # 对每个序列加入起始和分割等特殊标记（同时设置新列表来标记每个词属于第几句话）
  ...
  # 从词表中查询每个词的id来生成input_ids（方便后续做词嵌入）
  input_ids = tokenizer.convert_tokens_to_ids(tokens)

  # 设置对应的input_mask并补全（即有词的位置标为1，否则标为0，表示后续不再参与计算）
...
def main(_):
  ...
  if FLAGS.do_train:
    ...
    # 刚开始的num_warmup_steps次迭代学习率偏小，之后恢复原来设置的学习率
